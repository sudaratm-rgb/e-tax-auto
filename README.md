...
- รับ `contacts` เสริมได้ (`{"contacts": {"RE...": {"name": "ชื่อลูกค้า", "type": "juristic"}}}`)
  — บันทึกลง `send_log.contact_name`/`contact_type` เพื่อใช้เป็น fallback แสดงในตารางหลัก
  ภายหลัง เพราะใบที่ `alreadySent` แล้วจะไม่ถูกดึง contact ซ้ำอีก (ดูหัวข้อ "ความเร็วในการดึง
  ข้อมูล") ถ้าไม่บันทึกไว้ตอนนี้จะไม่มีทางเห็นชื่อ/ประเภทของใบนี้อีกเลย — หน้าเว็บส่ง `contacts`
  มาให้อัตโนมัติทั้งจากตารางหลัก (มีทั้งชื่อ+ประเภทที่ดึงมาแล้วในรอบนี้) และจากปุ่ม "sync" ใน
  Import Excel Report (มีแค่ชื่อจากไฟล์ Excel — ไม่มีคอลัมน์ประเภทลูกค้าในรายงาน PEAK)
  `POST /api/send-etax` เองก็รับ `contacts` แบบเดียวกันได้ เพื่อบันทึกตอนกดส่งจริงด้วย (ไม่งั้น
  ใบที่ส่งผ่านปุ่ม "ส่ง e-Tax" ปกติจะไม่มีชื่อ/ประเภทบันทึกไว้เลย)
...
ที่ตั้งใจ: หน้ารายละเอียดของใบที่ส่งแล้วจะไม่มีที่อยู่/เลขผู้เสียภาษีให้ดู (แสดง `-` พร้อม
ข้อความอธิบายว่าตั้งใจข้าม ไม่ใช่ข้อมูลหาย) — `contactCode` ยังมีอยู่เสมอเพราะมาจาก receipt
โดยตรง ไม่ต้องพึ่ง contact object ส่วน **ชื่อ + ประเภทลูกค้า** (นิติบุคคล/บุคคลธรรมดา) ยังพอ
มีให้เห็นได้ถ้าเคยบันทึกไว้ตอนกดส่งจริงหรือยืนยันด้วยมือ (`send_log.contact_name`/
`contact_type`, ดูหัวข้อ "บันทึกสถานะด้วยมือ") — `sentContactInfo()` ใน `lib/sendLog.ts`
ดึงมาให้ `buildRow` ใช้เป็น fallback แทน contact ที่ไม่ได้ดึง (เลือก entry ล่าสุด**ที่มีข้อมูล
นี้จริง** ไม่ใช่ entry ล่าสุดเฉย ๆ เพราะ entry จาก callback ที่มาทีหลังมักไม่มีชื่อ/ประเภทแนบมา
ด้วย ถ้าเลือกผิดจะได้ค่าว่างทับข้อมูลที่เคยมีจริง)

**ใบที่ส่งแล้วแต่ไม่เคยมีชื่อ/ประเภทบันทึกไว้เลย** (เช่น ส่งไปก่อนฟีเจอร์นี้จะมี หรือส่งผ่าน
PEAK UI ตรง ๆ ไม่เคย sync) จะไม่ถูกข้ามการดึง contact แบบเต็มร้อย — `/api/fetch` ดึง contact
ให้กลุ่มนี้ **ครั้งแรก** ครั้งเดียว แล้ว `sendLog.backfillContactInfo()` เขียนทับ (`UPDATE`,
ไม่ insert แถวใหม่ กันไปทับข้อความสถานะ "Success" เดิมในคอลัมน์หมายเหตุ) ลง entry ล่าสุดของ
code นั้นใน `send_log` — ครั้งต่อไปที่ดึงช่วงวันที่เดียวกัน โค้ดจะเจอใน `sentContactInfo()`
แล้วไม่ต้องดึง contact ซ้ำอีกเลย (เร็วตลอดไปหลังจากนั้น) เท่ากับว่าความช้าจากการ backfill
เกิดขึ้น **ครั้งเดียวต่อใบ** ไม่ใช่ทุกครั้งที่ดึง
